...
public class JavaScriptCompressor {
	...
	static final List<String> ones, twos, threes;
	...
	private static List<JavaScriptToken> parse(Reader in, ErrorReporter reporter)
	...
	private static void processStringLiterals(List<JavaScriptToken> tokens, boolean merge) {
	...
	*/
	private static void optimizeObjectMemberAccess(List<JavaScriptToken> tokens) {
	...
	private void optimizeLongKeywords(List<JavaScriptToken> tokens) {
		...
		for (i = 0, length = tokens.size(); i < length; i++) {
			...
			if (tokens.get(i).getType() == Token.DOT)
				if(i > 0 && i < length - 1 &&
					tokens.get(i - 1).getType() == Token.NAME &&
					tokens.get(i + 1).getType() == Token.NAME ) {
				token = tokens.get(i + 1);
				tv = token.getValue();
				if (isValidIdentifier(tv)) {
					if (tv.equals("prototype")) {
						tv = "[_P]";
						tokens.set(i + 1, new JavaScriptToken(Token.NAME, tv));
						tokens.remove(i);
						++i;
						length = length - 1;
					} else if (tv.equals("hasOwnProperty")) {
						tv = "[_H]";
						tokens.set(i + 1, new JavaScriptToken(Token.NAME, tv));
						tokens.remove(i);
						++i;
	 ...
	 */
	private static void optimizeObjLitMemberDecl(List<JavaScriptToken> tokens) {
		...
		for (i = 0, length = tokens.size(); i < length; i++) {
			if (tokens.get(i).getType() == Token.OBJECTLIT &&
					i > 0 && tokens.get(i - 1).getType() == Token.STRING) {
				token = tokens.get(i - 1);
				tv = token.getValue();
				tv = tv.substring(1, tv.length() - 1);
				if (isValidIdentifier(tv)) {
	...
	private List<JavaScriptToken> tokens;
	...
	private Map<Integer, ScriptOrFnScope> indexedScopes = new HashMap<Integer, ScriptOrFnScope>();
	...
	public void compress(Writer out, int linebreak, boolean munge, boolean verbose,
		...
		if (!disableOptimizations) {
			...
			optimizeLongKeywords(this.tokens);
		...
		out.write("var _P='prototype',_H='hasOwnProperty';"+sb.toString());
	}
